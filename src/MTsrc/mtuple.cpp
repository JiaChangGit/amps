...
int MTuple::InsertRule(Rule_DT_MT *rule) {
  uint32_t keys[5];
  for (int i = 0; i < prefix_dims_num; ++i)
  ...
  if (hash_table.InsertRule(rule, keys, hash) > 0) return 1;
  ++rules_num;
...
int MTuple::DeleteRule(Rule_DT_MT *rule) {
  uint32_t keys[5];
  for (int i = 0; i < prefix_dims_num; ++i)
    keys[i] = (uint64_t)rule->range[i][0] >> prefix_len_zero[i];
  uint32_t hash = HashKeys(keys, prefix_dims_num);

  if (hash_table.DeleteRule(rule, keys, hash) > 0) return 1;
  --rules_num;
  max_priority = hash_table.max_priority;
  return 0;
}
...
int MTuple::GetRules(vector<Rule_DT_MT *> &rules) {
...
int MTuple::Test(void *ptr) { return 0; }
