...
int prefix_dims_num = 2;  // to be 2 or 5
...
int MultilayerTuple::Create(vector<Rule_DT_MT *> &rules, bool insert) {
  pthread_mutex_init(&lookup_mutex, NULL);
  pthread_mutex_init(&update_mutex, NULL);

  if (tuple_layer == 0 || prefix_dims_num == 0) {
    std::cerr << "Wrong : MultilayerTuple tuple_layer " << tuple_layer
              << " prefix_dims_num " << prefix_dims_num << '\n';
...
void MultilayerTuple::InsertTuple(MTuple *tuple) {
  if (tuples_num == max_tuples_num) {
    MTuple **new_tuples_arr =
        (MTuple **)malloc(sizeof(MTuple *) * max_tuples_num * 2);
...
uint32_t MultilayerTuple::GetReducedPrefix(uint32_t *prefix_len,
                                           Rule_DT_MT *rule) {
...
int MultilayerTuple::InsertRule(Rule_DT_MT *rule) {
...
int MultilayerTuple::DeleteRule(Rule_DT_MT *rule) {
  uint32_t prefix_len[5];
  uint32_t prefix_pair = GetReducedPrefix(prefix_len, rule);
  map<uint32_t, MTuple *>::iterator iter = tuples_map.find(prefix_pair);
  MTuple *tuple = nullptr;
  if (iter != tuples_map.end()) {
    ...
    std::cout << ("Wrong: DeleteRule has no tuple\n");
    return 1;
  }
...
int MultilayerTuple::Lookup(Trace *trace, int priority) {
  ...
  for (int i = 0; i < tuples_num; ++i) {
    ...
    MHashNode *hash_node =
        tuple->hash_table.hash_node_arr[hash & tuple->hash_table.mask];
    while (hash_node) {
...
int MultilayerTuple::LookupAccess(Trace *trace, int priority,
                                  Rule_DT_MT *ans_rule,
  ...
  for (int i = 0; i < tuples_num; ++i) {
    ...
    while (hash_node) {
      ...
      if (hash_node->SameKey(keys)) {
        if (hash_node->has_next_multilayertuple) {
          priority = hash_node->next_multilayertuple->LookupAccess(
              trace, priority, ans_rule, program_state);
        } else {
          MRuleNode *rule_node = hash_node->rule_node;
          while (rule_node) {
            if (priority >= rule_node->priority) break;
...
int MultilayerTuple::Reconstruct() { return 0; }
...
int MultilayerTuple::CalculateState(ProgramState *program_state) {
  if (start_tuple_layer) program_state->tuples_num = tuples_num;
...
int MultilayerTuple::GetRules(vector<Rule_DT_MT *> &rules) {
...
int MultilayerTuple::Test(void *ptr) { return 0; }
