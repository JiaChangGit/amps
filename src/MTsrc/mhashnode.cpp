...
MHashNode::MHashNode(uint32_t *_keys, uint32_t _hash) {
  for (int i = 0; i < prefix_dims_num; ++i) keys[i] = _keys[i];
  hash = _hash;
  rules_num = 0;
  max_priority = 0;
  rule_node = nullptr;
  ...
  next_multilayertuple = nullptr;
}

bool MHashNode::SameKey(uint32_t *_keys) {
  for (int i = 0; i < prefix_dims_num; ++i)
    if (keys[i] != _keys[i]) return false;
  return true;
}

int MHashNode::InsertRule(Rule_DT_MT *rule, uint32_t tuple_layer) {
  if (has_next_multilayertuple) {
    if (next_multilayertuple->InsertRule(rule) > 0) {
      std::cout << ("next_multilayertuple->Insert fail\n");
      exit(1);
    }
    ++rules_num;
    if (rule->priority > max_priority) max_priority = rule->priority;
    return 0;
  }
  // insert rule_node list
  MRuleNode *insert_rule_node = new MRuleNode(rule);
  MRuleNode *pre_rule_node = nullptr;
  ...
  if (rules_num >= create_next_layer_rules_num && !has_next_multilayertuple &&
      tuple_layer < max_layers_num) {
    //  std::cout <<("Create next_multilayertuple\n");
    ...
    for (int i = 0; i < get_rules_num; ++i) InsertRule(rules[i], tuple_layer);
  }

  return 0;
}

int MHashNode::DeleteRule(Rule_DT_MT *rule, uint32_t tuple_layer) {
  ...
  if (has_next_multilayertuple && rules_num <= delete_next_layer_rules_num) {
    //  std::cout <<("delete next_multilayertuple\n");
    vector<Rule_DT_MT *> rules;
    GetRules(rules);
    int get_rules_num = rules.size();
    Free(false);
...
int MHashNode::GetRules(vector<Rule_DT_MT *> &rules) {
  MRuleNode *current_rule_node = rule_node, *next_rule_node = nullptr;
  ...
  if (has_next_multilayertuple) next_multilayertuple->GetRules(rules);
  return 0;
}
...
int MHashNode::Test(void *ptr) { return 0; }
