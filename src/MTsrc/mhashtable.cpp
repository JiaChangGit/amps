...
MHashNode *MHashTable::PickHashNode(uint32_t *keys, uint32_t hash) {
  ...
  MHashNode *pre_hash_node = nullptr;
  while (hash_node) {
...
int MHashTable::InsertRule(Rule_DT_MT *rule, uint32_t *keys, uint32_t hash) {
  MHashNode *hash_node = PickHashNode(keys, hash);
  if (!hash_node) hash_node = new MHashNode(keys, hash);
...
int MHashTable::DeleteRule(Rule_DT_MT *rule, uint32_t *keys, uint32_t hash) {
  MHashNode *hash_node = PickHashNode(keys, hash);
  if (!hash_node) {
...
int MHashTable::GetRules(vector<Rule_DT_MT *> &rules) {
  MHashNode *hash_node = nullptr, *next_hash_node = nullptr;
  for (int i = 0; i <= mask; ++i) {
    hash_node = hash_node_arr[i];
    while (hash_node) {
...
int MHashTable::Test(void *ptr) { return 0; }
