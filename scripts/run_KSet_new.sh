...
BUILD_DIR="./build"
EXECUTABLE="$BUILD_DIR/tests/KSet_new"
...
RUN_UPDATE_TEST=false  # 是否一併測 update（-u），預設關閉
...
run_update_test() {
    ...
    "$EXECUTABLE" -r "$ruleset_path" -p "$trace_path" -t -u > "$output_file"
