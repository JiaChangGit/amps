...
## How to use
## chmod +x scripts/per_run_PT.sh
## bash ./scripts/per_run_PT.sh
### Or
### sudo bash ./scripts/per_run_PT.sh

# 設定變數
BUILD_DIR="./build"
#############
TEST_PT_MAIN_EXEC="$BUILD_DIR/tests/PT_main"
#############
...
"$TEST_PT_MAIN_EXEC" -r "$RULESET_PATH" -p "$TRACE_PATH" > "$TEST_PT_MAIN_EXEC"_c_acl1.txt
"$TEST_PT_MAIN_EXEC" -r "$RULESET_PATH" -p "$TRACE_PATH" > "$TEST_PT_MAIN_EXEC"_c_acl1.txt
# echo "Running upd tests..."
# "$TEST_PT_MAIN_EXEC" -r "$RULESET_PATH" -p "$TRACE_PATH" -u > "$TEST_PT_MAIN_EXEC"_u_acl1.txt
