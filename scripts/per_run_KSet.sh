...
## How to use
## chmod +x scripts/per_run_KSet.sh
## bash ./scripts/per_run_KSet.sh
### Or
### sudo bash ./scripts/per_run_KSet.sh
...
"$TEST_KSET_MAIN_EXEC" -r "$RULESET_PATH" -p "$TRACE_PATH" -t -s > "$TEST_KSET_MAIN_EXEC"_c_acl1.txt
"$TEST_KSET_MAIN_EXEC" -r "$RULESET_PATH" -p "$TRACE_PATH" -t -s > "$TEST_KSET_MAIN_EXEC"_c_acl1.txt
# echo "Running upd tests..."
# "$TEST_KSET_MAIN_EXEC" -r "$RULESET_PATH" -p "$TRACE_PATH" -t -u > "$TEST_KSET_MAIN_EXEC"_u_acl1.txt
