...
## How to use
## chmod +x scripts/per_run_DBT.sh
## bash ./scripts/per_run_DBT.sh
### Or
### sudo bash ./scripts/per_run_DBT.sh

# 設定變數
BUILD_DIR="./build"
#############
TEST_DBT_MAIN_EXEC="$BUILD_DIR/tests/DBTable_main"
#############
...
"$TEST_DBT_MAIN_EXEC" -r "$RULESET_PATH" -p "$TRACE_PATH" > "$TEST_DBT_MAIN_EXEC"_c_acl1.txt
"$TEST_DBT_MAIN_EXEC" -r "$RULESET_PATH" -p "$TRACE_PATH" > "$TEST_DBT_MAIN_EXEC"_c_acl1.txt
# echo "Running upd tests..."
# "$TEST_DBT_MAIN_EXEC" -r "$RULESET_PATH" -p "$TRACE_PATH" -u > "$TEST_DBT_MAIN_EXEC"_u_acl1.txt
