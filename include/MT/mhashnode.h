...
struct MRuleNode {
  ...
  uint64_t MemorySize() { return sizeof(MRuleNode); }
...
struct MHashNode {
  ...
  int InsertRule(Rule_DT_MT *rule, uint32_t tuple_layer);
  int DeleteRule(Rule_DT_MT *rule, uint32_t tuple_layer);
  uint64_t MemorySize();
