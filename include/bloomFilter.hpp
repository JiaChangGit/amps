...
/**
 * @brief Bloom Filter 實現，支持單執行緒超高速查詢，64 位和 128 位數據
...
class BloomFilter {
 private:
  ...
  mutable std::ofstream
      log_file_;  // 日誌文件輸出流，mutable 允許 const 方法修改
#endif
  ...
  /**
   * @brief 獲取日誌輸出流
   * @return 指向 std::ostream 的引用（log_file_ 或 std::cout）
   */
  std::ostream& get_log_stream() const {
#ifdef ENABLE_LOG_TO_FILE
    return static_cast<std::ostream&>(log_file_);
#else
    return std::cout;
#endif
  }
 ...
 public:
...
#ifdef ENABLE_LOG_TO_FILE
      : log_file_("bloom_filter_log.txt")
#endif
  {
#ifdef ENABLE_LOG_TO_FILE
    // 檢查日誌文件是否成功打開
    if (!log_file_.is_open()) {
      throw std::runtime_error("無法打開日誌文件: bloom_filter_log.txt");
    }
#endif
    ...
    auto& log = get_log_stream();
    log << "初始化 Bloom Filter：\n";
    log << "預期元素數: " << expected_items << "\n";
    log << "誤報率: " << false_positive_rate * 100.0 << "%\n";
    log << "位陣列大小: " << size_ << " 位\n";
    log << "哈希函數數量: " << num_hashes_ << "\n";
    log << "種子基值: " << seed_base_ << "\n";
#endif
  }
  ...
  void insert(const T& data) {
    ...
    for (size_t i = 0; i < num_hashes_; ++i) {
      ...
      if (index / 8 >= bits_.size()) {
        std::cerr << "位陣列越界: index=" << index << ", byte=" << (index / 8)
...
#ifdef ENABLE_STATISTICS
    get_log_stream() << "插入數據，影響 " << bucket_count << " 個 bucket\n";
#endif
  }
  ...
  bool contains(const T& data) const {
      ...
      if (!(bits_[index / 8] & (1 << (index % 8)))) {
        ...
        int64_t duration =
            std::chrono::duration_cast<std::chrono::nanoseconds>(end - start)
                .count();
        get_log_stream() << "查詢耗時: " << duration << " 納秒 (未找到)\n";
#endif
        return false;
      }
    }
    ...
    int64_t duration =
        std::chrono::duration_cast<std::chrono::nanoseconds>(end - start)
            .count();
    get_log_stream() << "查詢耗時: " << duration << " 納秒 (可能存在)\n";
#endif
    return true;
  }
  ...
  /**
   * @brief 輸出 bucket 分布統計（僅在 ENABLE_STATISTICS 定義時有效）
   */
  void print_statistics() const {
#ifdef ENABLE_STATISTICS
    auto& log = get_log_stream();
    log << "Bloom Filter 統計資訊：\n";
    log << "位陣列大小: " << size_ << " 位\n";
    log << "哈希函數數量: " << num_hashes_ << "\n";
    log << "填充率: " << fill_ratio() * 100.0 << "%\n";
#endif
  }
};
